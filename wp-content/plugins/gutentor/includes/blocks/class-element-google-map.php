...
if ( ! class_exists( 'Gutentor_Element_Google_Map' ) ) {
	...
	class Gutentor_Element_Google_Map extends Gutentor_Block_Base{
		...
		function add_dynamic_css( $data, $attributes ) {
			...
			if ($attributes['gutentorBlockName'] != 'gutentor/e4') {
				return $data;
			}
            ...
            ' . gutentor_responsive_height_width('height',$attributes['e4Height']) . ' 
        }';
			$local_dynamic_css['desktop'] .= '#' . $attributes['id'] . '{
            ' . gutentor_responsive_height_width('height',$attributes['e4Height'], 'desktop') . ' 
        }';
			$local_dynamic_css['tablet']  .= '#' . $attributes['id'] . '{
            ' . gutentor_responsive_height_width('height',$attributes['e4Height'], 'tablet') . ' 
        }';
		...
		public function render_callback( $attributes, $content ) {
			...
			$output         = '<'.$tag.' class="'.apply_filters('gutentor_save_element_class', 'gutentor-element gutentor-element-google-map '.$align, $attributes).'" id="section-' . esc_attr($blockID) . '"   '.GutentorAnimationOptionsDataAttr($blockComponentAnimation).'>' . "\n";
